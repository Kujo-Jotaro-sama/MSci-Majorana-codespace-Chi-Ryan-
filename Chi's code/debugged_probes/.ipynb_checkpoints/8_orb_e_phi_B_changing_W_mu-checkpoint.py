...
def eta_B_scan(phi_array=[0, 4.*np.pi], B_array=[0., 14.7*mu_B*2./2./const.e*1000.], L_A=3700., a=6.0583*10, m=0.023, alpha=350., delta=0.182, eta=0.3):
    counter=0
    array_index = int(os.environ['PBS_ARRAY_INDEX'])
    W = np.sqrt(2.)*alpha/eta
    ...
    mu = bands_prelim(0.)[4]
    print('W=%.2g, mu=%.2g, eta=%.2g' % (W, mu, eta))
    for i in np.linspace(B_array[0], B_array[1], 101):
        counter+=1
        if (counter <= array_index*11) and (counter>(array_index-1)*11):
            energy_arrays=[]
            wavefunc_arrays=[]
            for j in np.linspace(phi_array[0], phi_array[1], 41):
                syst = make_system_mixed(L_A=L_A, W=W, a=a, m=m, alpha=alpha, mu=mu, delta=delta, B=i, phi=j, eta=None)
                energies, wavefunctions = find_boundstates(syst, -1.1*delta, 1.1*delta, rtol=1e-2*delta)
                energy_arrays.append(energies)
                wavefunc_arrays.append(wavefunctions)
