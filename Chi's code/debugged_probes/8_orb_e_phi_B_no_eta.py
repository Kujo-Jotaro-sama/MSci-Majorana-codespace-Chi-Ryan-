...
def B_phi_scan(phi_array=[0, 4.*np.pi], B_array=[0, 14.7*mu_B*2./2./const.e*1000.], L_A=3700., W=1400., a=6.0583*10, m=0.023, alpha=350., mu=0.887, delta=0.182, eta=None):
    counter=0
    array_index = int(os.environ['PBS_ARRAY_INDEX'])
    for i in np.linspace(B_array[0], B_array[1], 101):
        ...
        if (counter <= array_index*10) and (counter>(array_index-1)*10):
            ...
            for j in np.linspace(phi_array[0], phi_array[1], 41):
                syst = make_system_mixed(L_A=L_A, W=W, a=a, m=m, alpha=alpha, mu=mu, delta=delta, B=i, phi=j, eta=eta)
                energies, wavefunctions = find_boundstates(syst, -1.1*delta, 1.1*delta, rtol=1e-2*delta)
                energy_arrays.append(energies)
                wavefunc_arrays.append(wavefunctions)
