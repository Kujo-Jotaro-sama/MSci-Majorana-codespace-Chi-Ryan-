...
def B_phi_scan(phi_array=[0, 4.*np.pi], B_array=[0, 0.182*6.], L_A=1000., a=6.0583*10, m=0.023, alpha=350., mu=0., delta=0.182):
    counter=0
    array_index = int(os.environ['PBS_ARRAY_INDEX'])
    for i in np.linspace(B_array[0], B_array[1], 61):
        counter+=1
        if (counter <= array_index*7) and (counter>(array_index-1)*7):
            energy_arrays=[]
            wavefunc_arrays=[]
            for j in np.linspace(phi_array[0], phi_array[1], 41):
                syst = make_system_4orb(L_A=L_A, a=a, m=m, alpha=alpha, mu=mu, delta=delta, B=i, phi=j)
                energies, wavefunctions = find_boundstates(syst, -1.1*delta, 1.1*delta, rtol=1e-2*delta)
                energy_arrays.append(energies)
                wavefunc_arrays.append(wavefunctions)
            pickle.dump(energy_arrays, open('./data/e_B_index_%s' % (str(counter)), 'wb'))
            pickle.dump(wavefunc_arrays, open('./data/psi_B_index_%s' % (str(counter)), 'wb'))
            print('i=%i' % (i))
            print(energy_arrays)
